...
static int dmc_pub_addr_remap(void)
{
	...
	if ((ddr_chip_cur.cs_num == 1) && (chipsize == 0x30000000))
	{
		dmc_portx_remap_addr_x(0x02100210, 0x02100210);
		return 0;
	}
...
int sdram_test(int start, int end)
{
	int ret;

	if(start <= ddr_chip_cur.cs0_jedec_info->cs_size) {
		ret = ddr_scan_fdl1_test(0,start);
		if(ret != 0)
			return -1;
	} else {
		ret = ddr_scan_fdl1_test(0,ddr_chip_cur.cs0_jedec_info->cs_size);
		if(ret != 0)
			return -1;
		ret = ddr_scan_fdl1_test(ddr_chip_cur.cs0_jedec_info->cs_size, start);
		if(ret != 0)
			return -1;
	}
	...
	if(end >= ddr_chip_cur.cs0_jedec_info->cs_size) {
		if(ddr_chip_cur.cs_num == 2) {
			ret = ddr_scan_fdl1_test(end, 0);
			if(ret != 0)
				return -1;
		}
	} else {
		ret = ddr_scan_fdl1_test(end,ddr_chip_cur.cs0_jedec_info->cs_size);
		if(ret != 0)
			return -1;
		if(ddr_chip_cur.cs_num == 2) {
			ret = ddr_scan_fdl1_test(ddr_chip_cur.cs0_jedec_info->cs_size, 0);
			if(ret != 0)
				return -1;
		}
	}
