...
void ddr_freq_target_set(uint32 ddr_clk)
{
	...
	}else
	{
		switch(ddr_freq)
		{
			...
			case 500  :fn_ctl_target=1;fsp_wr=1;fsp_op=1;break;
			case 250  :fn_ctl_target=0;fsp_wr=1;fsp_op=1;break;
			default:while(1);
			#else
...
{
	reg_bit_set(CANDS_PI0_(137), 16, 7, 0x4c);     //pi_calvl_vref_initial_start_point	40% range1
	reg_bit_set(CANDS_PI0_(137), 24, 7, 0x6e);     //pi_calvl_vref_initial_stop_point	60% range1
...
//_____________________________commen config for lp3/lp4x____________________________
	...
	if(!islp3)
	{
		...
		for(i=0;i<4;i++)
		{
			reg_bit_set(CANDS_PHY0_(89)+i*0x200,0,1,0x0);
			reg_bit_set(CANDS_PHY0_(88)+i*0x200,0,10,0x180);
		}
		reg_bit_set(CANDS_PHY0_(1024),8,3,0x2);
		for(i=0;i<4;i++)
