...
struct RE_REINTI_TEST_MODE *reint_test_mode_addr = REINIT_TEST_MODE_ADDR;
...
{
    reint_test_mode_addr->interval_s = 1;
    reint_test_mode_addr->interval = short_interval;
    reint_test_mode_addr->bist = bist_after_reinit;
    reint_test_mode_addr->dfs = no_dfs;
}
...
//	reg_bit_set(DMC_CTL_(81), 16, 1, 0X1);//enable cpst period en
	...
	//set qos
        reg_num_tmp = (sys_info->freq_num * 20) + 9;
	reg_bit_set(DMC_PHY0_(reg_num_tmp),31,1,0x1);// only lp3 RPULL
	reg_bit_set(DMC_PHY1_(reg_num_tmp),31,1,0x1);// only lp3 RPULL
...
uint32 pub_power_on()
{
	...
	#endif
//	bist_test(BIST_READ,SIPI_PATT, NORMAL_MODE, BIST_LEN, BIST_CHN0_START_ADDR, BIST_CHN1_START_ADDR, BIST_CHN2_START_ADDR);
        dmc_ddr_test(0);
	pub_acc_rdy_set();
}
...
void DMC_ReInit(void)
{
    ...
    while(1)
    {
        ...
        #ifdef REINIT_LOG_EN
        ddr_print("\r\npub poweroff num:");
//        ddr_print_u32_hex(reinit_info_g->pub_poweroff_num);
        ddr_print_u32_dec(reinit_info_g->pub_poweroff_num);
        ddr_print("\r\npub poweroff time consumpution:");
//        ddr_print_u32_hex(reinit_info_g->pub_poweroff_time);
        ...
        #ifdef REINIT_LOG_EN
        ddr_print("\r\npub poweron num:");
//        ddr_print_u32_hex(reinit_info_g->pub_poweron_num);
        ddr_print_u32_dec(reinit_info_g->pub_poweron_num);
        ddr_print("\r\npub poweron time consumpution:");
//        ddr_print_u32_hex(reinit_info_g->pub_poweron_time);
